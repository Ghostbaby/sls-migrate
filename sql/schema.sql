...
CREATE TABLE IF NOT EXISTS alerts (
    ...
    configuration_id BIGINT COMMENT '配置ID，关联alert_configurations表',
    schedule_id BIGINT COMMENT '调度ID，关联alert_schedules表',
...
CREATE TABLE IF NOT EXISTS alert_configurations (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    alert_id BIGINT NOT NULL COMMENT '关联的Alert ID',
    ...
    condition_config_id BIGINT COMMENT '条件配置ID',
    group_config_id BIGINT COMMENT '分组配置ID',
    policy_config_id BIGINT COMMENT '策略配置ID',
    template_config_id BIGINT COMMENT '模板配置ID',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '记录更新时间',
...
CREATE TABLE IF NOT EXISTS alert_queries (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    alert_id BIGINT NOT NULL COMMENT '关联的Alert ID',
    query_string TEXT NOT NULL COMMENT '查询语句',
    query_type VARCHAR(100) COMMENT '查询类型',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '记录更新时间',
    FOREIGN KEY (alert_id) REFERENCES alerts(id) ON DELETE CASCADE,
    INDEX idx_alert_id (alert_id),
...
CREATE TABLE IF NOT EXISTS group_configurations (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    group_by_fields TEXT COMMENT '分组字段，JSON格式',
    group_interval VARCHAR(50) COMMENT '分组间隔',
    group_wait VARCHAR(50) COMMENT '分组等待时间',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '记录更新时间'
...
CREATE TABLE IF NOT EXISTS policy_configurations (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    policy_name VARCHAR(255) COMMENT '策略名称',
    policy_type VARCHAR(100) COMMENT '策略类型',
    policy_config TEXT COMMENT '策略配置，JSON格式',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '记录更新时间'
...
CREATE TABLE IF NOT EXISTS severity_configurations (
    id BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
    alert_config_id BIGINT NOT NULL COMMENT '关联的Alert配置ID',
    severity_level VARCHAR(50) NOT NULL COMMENT '严重程度级别',
    severity_config TEXT COMMENT '严重程度配置，JSON格式',
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP COMMENT '记录创建时间',
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '记录更新时间',
